...
namespace Omnix.Network.Connection.Tests
{
    public class BaseNonblockingConnectionTests
    {
        ...
        public void RandomSendAndReceiveTest()
        {
            ...
            using (var connection1 = new BaseConnection(new SocketCap(socket1, false), 1024 * 1024 * 256, BufferPool.Shared))
            using (var connection2 = new BaseConnection(new SocketCap(socket2, false), 1024 * 1024 * 256, BufferPool.Shared))
            {
