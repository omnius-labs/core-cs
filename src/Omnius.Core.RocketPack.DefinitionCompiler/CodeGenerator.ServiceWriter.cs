...
namespace Omnius.Core.RocketPack.DefinitionCompiler
{
    internal partial class CodeGenerator
    {
        private sealed class ServiceWriter
        {
            ...
            {
                b.WriteLine($"{_accessLevel} interface I{ serviceDefinition.Name}");
                b.WriteLine("{");
                ...
                using (b.Indent())
                {
                    foreach (var func in serviceDefinition.Elements)
                    {
                        if (func.InType is not null && func.OutType is not null)
                        {
                            ...
                            {
                                b.WriteLine($"{GenerateTypeFullName("ValueTask<>", outTypeObjectDef.FullName)} {func.Name}({inTypeObjectDef.FullName} param);");
                            }
                        }
                    }
                }
            ...
            {
                var interfaceName = "global::" + serviceDefinition.Namespace + "." + "I" + serviceDefinition.Name;
                var className = serviceDefinition.Name + "Sender";

                b.WriteLine($"{_accessLevel} class { className } : { interfaceName }");
                b.WriteLine("{");
                using (b.Indent())
                {
                    foreach (var func in serviceDefinition.Elements)
                    {
                        if (func.InType is not null && func.OutType is not null)
                        {
                            if (this.FindDefinition(func.InType) is ObjectDefinition inTypeObjectDef
                                && this.FindDefinition(func.OutType) is ObjectDefinition outTypeObjectDef)
                            {
                                b.WriteLine($"public async {GenerateTypeFullName("ValueTask<>", outTypeObjectDef.FullName)} {func.Name}({inTypeObjectDef.FullName} param)");
                                b.WriteLine("{");
            ...
            public void Write_Server(CodeBuilder b, ServiceDefinition serviceDefinition)
            {
                ...
                using (b.Indent())
                {
                    b.WriteLine($"private readonly { interfaceName } _impl;");
                    b.WriteLine($"public { className }({ interfaceName } impl)");
                    b.WriteLine("{");
                    using (b.Indent())
                    {
